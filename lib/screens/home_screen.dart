...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _initialiser() async {
    ...
    SocketService.ecouterPositions((data) {
      ...
      setState(() {
        ...
        final membreMAJ = MembrePosition(
          ...
          role:
              data['role'] ??
              'membre', // <-- correctif : respecte le role reçu (ex. 'enfant' pour un bracelet)
    ...
    SocketService.ecouterSos((data) {
      ...
      setState(() {
        _dernierSosMessage =
            'SOS déclenché par le bracelet $braceletId\nCoordonnées: $lat, $lng\nNiveau: $severity';
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('🚨 SOS déclenché par le bracelet $braceletId'),
  ...
  /// Recentre / dézoome la carte pour que le parent ET tous les membres
  /// (dont le bracelet enfant) restent visibles, sans avoir à zoomer à la main.
  void _ajusterVue() {
    final points = <LatLng>[
      if (_maPosition != null)
          ...
          _chargement
              ...
              : FlutterMap(
                ...
                children: [
                  ...
                  // Lien pointillé entre le parent (admin) et chaque enfant (bracelet)
                  if (_maPosition != null)
                    PolylineLayer(
                      polylines:
                          _membresFiltres
